...
/* Translate Emacs based prefix commands into internal keycodes. */
int interpret_ctrl_x(void)
{
	int input = get_kbinput(midwin, VISIBLE);
	...
	switch (input) {
		case '\x03':
			return EMACS_EXIT;
		case '\x06':
			return EMACS_INSERTFILE;
		case '\x13':
			return EMACS_SAVEFILE;
		case '\x15':
			return EMACS_UNDO;
		case '\x17':
...
void process_a_keystroke(void)
{
	...
	if (!ISSET(MODERN_BINDINGS) && input == '\x18') {
		input = interpret_ctrl_x();
#ifndef NANO_TINY
		if (input == THE_WINDOW_RESIZED)
			return;
#endif
...
int main(int argc, char **argv)
{
	...
	/* Emacs based defaults and --modernbindings need ^Q and ^S functional. */
	UNSET(PRESERVE);
