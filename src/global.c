...
#define SLASH_OR_DASH  "^/"
	...
	if (ISSET(MODERN_BINDINGS)) {
		...
		add_to_sclist(MMAIN, "^S", 0, do_search_forward, 0);
		add_to_sclist(MMAIN, "^R", 0, do_search_backward, 0);
		add_to_sclist(MMAIN, "^X^C", EMACS_EXIT, do_exit, 0);
		add_to_sclist(MMAIN, "^X^S", EMACS_SAVEFILE, do_savefile, 0);
		add_to_sclist(MMAIN, "^X^W", EMACS_WRITEOUT, do_writeout, 0);
		add_to_sclist(MMAIN, "^X^F", EMACS_INSERTFILE, do_insertfile, 0);
		add_to_sclist(MMAIN, "^Q", 0, do_verbatim_input, 0);
		...
		add_to_sclist(MMAIN|MBROWSER|MHELP|MLINTER, "M-V", 0, do_page_up, 0);
	...
	if (ISSET(MODERN_BINDINGS)) {
		...
		add_to_sclist(MMAIN, "M-%", 0, do_replace, 0);
	}
#ifdef NANO_TINY
